...
train_data <- ts(aep_data_daily$AEP_MW[1:split_point], start = c(2004, 10), frequency = 365.25)
test_data <- ts(aep_data_daily$AEP_MW[(split_point + 1):nrow(aep_data_daily)], start = c(2004, 10), frequency = 365.25)
...
# Fit ARIMA with parallelism and restricted parameters
plan(multisession)
arima_model <- auto.arima(train_data, max.p = 5, max.q = 5, max.d = 1, parallel = TRUE, stepwise = TRUE)
